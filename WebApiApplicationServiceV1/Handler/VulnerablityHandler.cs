...
namespace WebApiApplicationService.Handler
{
    ...
    public class VulnerablityHandler : IScopedVulnerablityHandler, ISingletonVulnerablityHandler
    {
        ...
        public VulnerablityHandler(string host = "127.0.0.1", int port = 3310, bool deleteFilesPermantly = true)
        {
            _logger = new LoggerFactory().CreateLogger<VulnerablityHandler>();
            Host = host;
            Port = port;
            string quarantineDirectory = AppPaths.AvQuarantinePath;
        ...
        private async void Init()
        {
            ...
            if(!ping)
            {
                _logger.Logging(LogLevel.Error,"antivirus host not available, host seems down", MethodBase.GetCurrentMethod(),this.GetType().Name);
            }
            else
            {
                _logger.Logging(LogLevel.Information, "antivirus host successfully connected", MethodBase.GetCurrentMethod(), this.GetType().Name);
