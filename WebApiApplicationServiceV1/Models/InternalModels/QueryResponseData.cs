        ...
        public bool HasStorageData
        {
            get
            {
                return (_dataStorage != null)?
                    (_dataStorage.Count != 0 && this.HasData && !this.HasErrors?true:false):false;
            }
        }
        #region Ctor
    ...
    public class QueryResponseData : IDisposable
    {
        ...
        public List<SchemaNode> StatementIncludedDbNodes
        {
            get
            {
                ...
                SQLDefinitionProperties.SQL_STATEMENT_ART type = this.StatementArt;
                ...
                if (statementTmp != null && type != SQLDefinitionProperties.SQL_STATEMENT_ART.NONE)
                {
                    ...
                    splitList = statementTmp.Split(new string[] { " " }, StringSplitOptions.RemoveEmptyEntries).ToList<string>();
        ...
        public SQLDefinitionProperties.SQL_STATEMENT_ART StatementArt
        {
            get
            {
                return GetStatementArt(this.Statement);
            }
        }
                ...
                if (QueryParametes != null)
                {
                    if (QueryParametes.Count != 0)
                    {
                        foreach (MySqlParameter item in QueryParametes)
                        {
                            ...
                            bool findInStatement = this.Statement.IndexOf(valueDescriptor) == GeneralDefs.NotFoundResponseValue ?
                                false : true;
                            if (findInStatement)
                            {
        ...
        public bool HasStatementJoins
        {
            get
            {
                List<SchemaNode> tmp = this.StatementIncludedDbNodes.FindAll(x =>
        ...
        public SchemaNode PrimaryNode
        {
            get
            {
                ...
                List<SchemaNode> tmp = this.StatementIncludedDbNodes;
