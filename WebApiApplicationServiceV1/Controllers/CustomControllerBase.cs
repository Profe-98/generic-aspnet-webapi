...
namespace WebApiApplicationService
{
    ...
    public class CustomControllerBase : ControllerBase,IDisposable
    {
        ...
        public string ControllerName
        {
            get
            {
                string tmpName = this.GetType().Name.ToLower();
        ...
        internal ActionDescriptor GetMatchingAction(string path, string httpMethod)
        {
        ...
        internal bool MatchesTemplate(string routeTemplate, string requestPath)
        {
        ...
        internal bool CheckGuid(string guid)
        {
        ...
        internal AreaAttribute GetArea()
        {
            Type classType = this.GetType();
            if (classType != null)
            {
                System.Attribute attribute = System.Attribute.GetCustomAttribute(classType, typeof(AreaAttribute));
                if (attribute != null)
                {
        ...
        internal List<RouteAttribute> GetRoutes()
        {
            List<RouteAttribute> routeAttributes = new List<RouteAttribute>();
            Type classType = this.GetType();
            if (classType != null)
            {
                System.Attribute attribute = System.Attribute.GetCustomAttribute(classType, typeof(RouteAttribute));
                if (attribute != null)
                {
                    routeAttributes.Add((RouteAttribute)attribute);
                }
            }
            List<RouteAttribute> tmp = GetMethodAttributesGeneric<RouteAttribute>(this.GetType());
        ...
        internal List<HttpMethodAttribute> GetHttpMethods()
        {
            return GetMethodAttributesGeneric<HttpMethodAttribute>(this.GetType());
        }
        internal List<HttpMethodAttribute> GetHttpMethods(Type type)
        {
            return GetMethodAttributesGeneric<HttpMethodAttribute>(type);
        }
        private List<T> GetAttributesGeneric<T>(Type type) where T : System.Attribute
        {
            ...
            for (int j = 0; j < parentModelProperties.Count; j++)
            {
                ...
                List<System.Attribute> attributes = item.GetCustomAttributes(typeof(T)).ToList();
        ...
        private List<T> GetMethodAttributesGeneric<T>(Type type) where T : System.Attribute
        {
        ...
        internal JsonApiObjectResult JsonApiResult(List<ApiDataModel> value, HttpStatusCode httpStatusCode, string message = null, string debugMessage = null, object debugObj = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
        ...
        internal JsonApiErrorResult JsonApiErrorResult(List<ApiErrorModel> value, HttpStatusCode httpStatusCode, string message = null, string debugMessage = null, object debugObj = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            bool devMode = _webHostEnvironment.IsDevelopment();
        ...
        internal static JsonApiErrorResult JsonApiErrorResultS(List<ApiErrorModel> value, HttpStatusCode httpStatusCode, string message = null, string debugMessage = null, object debugObj = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            JsonApiErrorResult result = new JsonApiErrorResult(value, message);
            result.StatusCode = (int)httpStatusCode;
        ...
        internal JsonApiErrorResult JsonApiErrorResult(ApiRootNodeModel value, HttpStatusCode httpStatusCode, string message = null, string debugMessage = null, object debugObj = null)
        {
            bool devMode = _webHostEnvironment.IsDevelopment();
        ...
        internal static JsonApiErrorResult JsonApiErrorResultS(ApiRootNodeModel value, HttpStatusCode httpStatusCode, string message = null, string debugMessage = null, object debugObj = null)
        {
            JsonApiErrorResult result = new JsonApiErrorResult(value.Errors, message);
            result.StatusCode = (int)httpStatusCode;
            value.Jsonapi = ApiRootNodeModel.GetApiInformation();
    ...
    public static class CustomControllerBaseExtensions
    {
        ...
        {
            string headerValue = HeaderValueGet(context, "Authorization");
            if(headerValue != null)
            {
                string token = null;
        ...
        public static ILogger TraceHttpTraffic(this ILogger logger, MethodBase executedMethod, HttpContext httpContext, string caller)
        {
            ...
            if (httpContext.Response.HasStarted)
            {
                ...
                logger?.Logging(LogLevel.Information, tmp,executedMethod,caller, WebApiApplicationService.Logging.CustomLogEvents.HttpResponse);
            }
            else
            {
                ...
                logger?.Logging(LogLevel.Information, tmp, executedMethod, caller, WebApiApplicationService.Logging.CustomLogEvents.HttpRequest);
            }
            return logger;
        }
        public static ILogger Logging(this ILogger logger,LogLevel logLevel,string msg, MethodBase executedMethod, string caller,int eventId = WebApiApplicationService.Logging.CustomLogEvents.GeneralInfo)
        {
        ...
        public static async void RegisterBackend(this IEndpointRouteBuilder endpoints,IWebHostEnvironment env, ISingletonDatabaseHandler databaseHandler,IActionDescriptorCollectionProvider actionDescriptorCollectionProvider,IConfiguration configuration)
        {
            ApiModule apiModule = new ApiModule(databaseHandler);
            ...
            if (c.HasData)
            {
                foreach (ClassModel classModel in c.DataStorage)
                {
                    ...
                    {
                        foreach (System.Data.DataRow row in constraints.Data.Rows)
                        {
            ...
            if (data.HasData)
            {
                foreach (var item in data.DataStorage)
                {
                    ...
                    if (!SQLDefinitionProperties.BackendTablesEx.ContainsKey(item.TableName))
                    {
                        ...
                        if (nettype != null)
                            SQLDefinitionProperties.BackendTablesEx.Add(item.TableName, new ClassModelWrapper(d, nettype));
                    }
                    else if (item.EntityOne != null)
                    {
            ...
            foreach (ControllerActionDescriptor controlller in controllers)
            {
                ...
                if (ns.StartsWith(env.ApplicationName) && controlller.ControllerTypeInfo.Name.EndsWith("Controller") && !workItems.Contains(controllerType))
                {
                    ...
                    if (ctorInfo.Length == 1)
                    {
                        ...
                        CustomControllerBase customControllerBase = (dynamic)repository;
