...
namespace WebApiApplicationService.Models
{
    ...
    public class ApiDataModel
    {
        ...
        public Type NetType
        {
            get
            {
                return this.Type == null ?
                    null : SQLDefinitionProperties.GetBackendTableByTypeStr(this.Type);
            }
        }
        [JsonIgnore]
        public object Instance
        {
            get
            {
                return Activator.CreateInstance(this.NetType);
            }
        }
                ...
                if (value != null)
                {
                    ...
                    if (t == typeof(JsonElement))
                    {
                        ...
                        using (JsonHandler _jsonHandler = new JsonHandler(true))
                        {
                            ...
                            {
                                Type conversionType = this.NetType;
        ...
        { 
            get
            {
                if(_link == null || _link.Self == null)
                {
                    ...
                    _link.Self = (this.Attributes == null?
                        "" : (("/" + AreaParent) + "/" + Type.Replace("model", "") + "/" + (this.Id == Guid.Empty?String.Empty: this.Id) + ""));
                }
                    ...
                    foreach (ApiDataModel item in _relations)
                    {
                        ...
                        if (copyList.Count != 0)
                        {
                            foreach (var it in copyList)
                            {
                                ...
                                bool canAdd = true;
                                int maxParentDisplayDepth = this.MaxDisplayNodeDepth;
        ...
        public async Task<List<ApiDataModel>> SetRelationMaxDepth(int maxDepth, ApiDataModel currentDepthNode, List<ApiDataModel> currentProgress)
        {
            ...
            if (this.RelationshipsInternal != null)
            {
        ...
        {
            List<ApiDataModel> responseValue = currentProgress;
            if (directionDownwardToChild? this.RelationshipsInternal != null: this.RelationshipsInternalParent != null)
            {
                if (directionDownwardToChild?this.RelationshipsInternal.Count != 0: this.RelationshipsInternalParent.Count != 0)
                {
                    foreach (ApiDataModel item in directionDownwardToChild?this.RelationshipsInternal:this.RelationshipsInternalParent)
                    {
                        item.MaxDisplayNodeDepth = _maxDepth;
                        item.LinkVisbility = false;
                        responseValue.Add(item);
