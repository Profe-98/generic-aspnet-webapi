...
namespace WebApiApplicationService.Handler
{
    ...
    public class DatabaseHandler : IScopedDatabaseHandler, ISingletonDatabaseHandler, ITransientDatabaseHandler
    {
        ...
        public bool AutoCommit {
            ...
            set
            {
                ...
                if(value == false)
                {
                    SetEnvironmentVar<int>(MYSQL_ENV_VARS.AUTO_COMMIT,1,0,false);
        ...
        public DatabaseHandler(IAppconfig appconfig)
        {
            InitODBCHandler(appconfig.AppServiceConfiguration.DatabaseConfigurationModel.MysqlConnectionString);
            this.AutoCommit = appconfig.AppServiceConfiguration.DatabaseConfigurationModel.AutoCommit;

        }
        public DatabaseHandler(MySqlConnectionStringBuilder connectionString)
        {
        ...
        public string ConvertArrayToWhereInFieldArray<T>(T[] arrayOrList) where T : struct
        {
            ...
            return ConvertListToWhereInFieldArray<T>(arrayOrList.ToList());
        }
        /// <summary>
        ...
        public async Task<bool> OpenAsync()
        {
            if (_odbcConnection.State != ConnectionState.Open)
            {
                try
                {
                    ...
                    this.IsConnectionAvaible = true;
                    return true;
                }
                catch (Exception ex)
                {
                    this.IsConnectionAvaible = false;
        ...
        {
            return await this.ExecuteQuery("SELECT UUID();");
        }
        /// <summary>
        /// Last inserted id queried by user (only available for tables w/ auto_increment id column)
        /// </summary>
        /// <returns></returns>
        public async Task<UInt64> LastInsertId()
        {
            QueryResponseData response = await this.ExecuteQuery("SELECT LAST_INSERT_ID();");
            return response.HasData ?
                (UInt64)response.Data.Rows[0].ItemArray[0] :UInt64.MinValue;
        }
        public async Task<QueryResponseData<T>> ExecuteQuery<T>(string query, DbTransaction transaction = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            ...
            QueryResponseData tmpData = await this.ExecuteQuery(query,null,responseData,transaction: transaction,file:file,member:member,line:line);
        ...
        private async Task<QueryResponseData<T>> FillGenericDataSet<T>(QueryResponseData queryResponseData)
        {
            ...
            if (queryResponseData.StatementArt == SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT)
            {
                string insertResultGetQuery = "SELECT * FROM " + queryResponseData.PrimaryNode.NodeFullyName + " WHERE " + SQLDefinitionProperties.GeneralUuidFieldName + " = '" + queryResponseData.LastInsertedId + "';";
        ...
        public async Task<QueryResponseData<T>> ExecuteQueryWithMap<T>(string query, T objectInstance, DbTransaction transaction = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            QueryResponseData<T> responseData = new QueryResponseData<T>(query);
            QueryResponseData tmpData = await this.ExecuteQuery(query, objectInstance, responseData, transaction: transaction, file: file, member: member, line: line);
            responseData = (QueryResponseData<T>)tmpData;
        ...
        public async Task<QueryResponseData> ExecuteQuery<T>(string query,T objectInstance,QueryResponseData queryResponseData, DbTransaction transaction = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            ...
            _databaseAttributeManager.FetchObjectToCommandParameters(objectInstance, ref mySqlCommand, SQLDefinitionProperties.SQL_STATEMENT_ART.NONE);
            int indexOfUuidField = GeneralDefs.NotFoundResponseValue;
            if (mySqlCommand.Parameters != null)
            {
                if (queryResponseData.StatementArt == SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT)
                {
                    indexOfUuidField = mySqlCommand.Parameters.IndexOf("@" + SQLDefinitionProperties.GeneralUuidFieldName);
            ...
            return await this.ExecuteQuery(query, mySqlCommand, queryResponseData, indexOfUuidField, transaction: transaction, file: file, member: member, line: line);
        }
        ...
        public QueryResponseData ExecuteQuerySync(string query, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0)
        {
            ...
            if (_threadSafetyDataReader)
                System.Threading.Monitor.Enter(MonitorLockObject);//wenn über BackgroundService Actions mit Query Execution ausgeführt werden sollen, muss DbDataReader durch lock von Multitask/Multithread Zugriff geschützt werden, da der DbDataReader nur mit einer Connection Instance readen kann
            ...
            if (_threadSafetyDataReader && System.Threading.Monitor.IsEntered(MonitorLockObject))
                System.Threading.Monitor.Exit(MonitorLockObject);
                ...
                try
                {
                        ...
                        !responseValue .IsTransactionalOperation? new MySqlCommand(query, _odbcConnection):new MySqlCommand(query,_odbcConnection, (MySqlTransaction)transaction) : mySqlCmd)
                    {
                        ...
                        if(mySqlCmd != null && uuidIndex != GeneralDefs.NotFoundResponseValue)
                        {
                            ...
                            if (responseValue.StatementArt == SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT)
                            {
        ...
        public async Task<QueryResponseData<T>> ExecuteQueryWithDapper<T>(string query,object param = null, [CallerFilePath] string file = "", [CallerMemberName] string member = "", [CallerLineNumber] int line = 0) where T : UserModel
        {
            ...
            if (openedConnection)
            {
                ...
                try
                {
                    /*var data = await _odbcConnection.QueryAsync<T,WebApiApplicationService.Models.UserRelationRoleModel,T>(query, (u,r) => {
