...
namespace WebApiApplicationService
{
        ...
        where T3 : GeneralMimeFileFormData
    {
        ...
        internal string FileSystemAttachmentStorePath
        {
            get
            {
                return Path.Combine(AppPaths.RootDir, AppResources.DownloadFolderName, ControllerNameFully.ToLower()); 
            }
        }

        public CustomApiControllerBase(ILogger<CustomApiControllerBase<T, T2>> logger, IScopedVulnerablityHandler vulnerablityHandler, IMailHandler mailHandler, IAuthHandler authHandler, IScopedDatabaseHandler databaseHandler, IJsonApiDataHandler jsonApiHandler, ITaskSchedulerBackgroundServiceQueuer queue, IScopedJsonHandler jsonHandler, ICachingHandler cache, IActionDescriptorCollectionProvider actionDescriptorCollectionProvider, IWebHostEnvironment env,IConfiguration configuration,IFileHandler fileHandler,IRabbitMqHandler rabbitMqHandler,IAppconfig appConfig, INodeManagerHandler nodeManagerHandler, IScopedEncryptionHandler scopedEncryptionHandler) :
        ...
        public CustomApiControllerBase(ILogger<CustomApiControllerBase<T, T2>> logger, IScopedVulnerablityHandler vulnerablityHandler, IMailHandler mailHandler, IAuthHandler authHandler, IScopedDatabaseHandler databaseHandler, IJsonApiDataHandler jsonApiHandler, ITaskSchedulerBackgroundServiceQueuer queue, IScopedJsonHandler jsonHandler, ICachingHandler cache, IWebHostEnvironment env,IConfiguration configuration, IFileHandler fileHandler, IRabbitMqHandler rabbitMqHandler, IAppconfig appConfig, INodeManagerHandler nodeManagerHandler, IScopedEncryptionHandler scopedEncryptionHandler) :
            base(logger, vulnerablityHandler, mailHandler, authHandler, databaseHandler,  jsonApiHandler, queue, jsonHandler, cache, env, configuration,rabbitMqHandler,appConfig,nodeManagerHandler, scopedEncryptionHandler)
        {
            _fileHandler = fileHandler;
            InitStorageFolder();
        }
        ...
        {
            Func<T, string, System.Threading.Tasks.Task<ActionResult>> f = (x, y) => Utils.CallAsyncFunc<T, string, ActionResult>(x, y, async (x, y) =>
            {
        ...
        [NonAction]
        internal async Task<ActionResult<ApiRootNodeModel>> ExecBodyMethodForPutFile(string id,T3 formBodyObject,List<IFormFile> formFiles,Func<T, List<IFormFile>, System.Threading.Tasks.Task<ActionResult<ApiRootNodeModel>>> methodBodyExecMethod)
        {
        ...
        [NonAction]
        internal async Task<ActionResult> ExecBodyMethodForGetFile(string id, Func<T, string, System.Threading.Tasks.Task<ActionResult>> methodBodyExecMethod, string file = null)
        {
            WebApiApplicationService.Models.InternalModels.MethodDescriptor methodInfo = _webHostEnvironment.IsDevelopment() ? new WebApiApplicationService.Models.InternalModels.MethodDescriptor { c = this.GetType().Name, m = MethodBase.GetCurrentMethod().Name } : null;
            if (methodBodyExecMethod == null)
            {
        ...
        [NonAction]
        internal async Task<ActionResult> PutFilesOnFileSys(List<FileLocationDescriptor> files)
        {
            List<ApiErrorModel> errors = new List<ApiErrorModel>();
        ...
        [NonAction]
        internal async Task<ActionResult> GetArchivedFile(T data, string file, string zipArchivPath, FileLocationDescriptor[] zipContainingContentFiles)
        {
            WebApiApplicationService.Models.InternalModels.MethodDescriptor methodInfo = _webHostEnvironment.IsDevelopment() ? new WebApiApplicationService.Models.InternalModels.MethodDescriptor { c = this.GetType().Name, m = MethodBase.GetCurrentMethod().Name } : null;
        ...
        where T2 : CustomBackendModule<T>
    {
        ...
        protected readonly ILogger<CustomApiControllerBase<T, T2>> _logger;
        ...
        private readonly Func<WebApiApplicationService.Models.RabbitMQ.MessageModel, MessageModelResponse> _generalFunc = new Func<Models.RabbitMQ.MessageModel, MessageModelResponse>((x) =>
        {
            return new MessageNOK();
        });
        private readonly Func<WebApiApplicationService.Models.RabbitMQ.MessageModel, MessageModelResponse> _entityCreateFunc = new Func<Models.RabbitMQ.MessageModel, MessageModelResponse>((x) =>
        {
            return new MessageNOK();
        });
        private readonly Func<WebApiApplicationService.Models.RabbitMQ.MessageModel, MessageModelResponse> _entityUpdateFunc = new Func<Models.RabbitMQ.MessageModel, MessageModelResponse>((x) =>
        {
            return new MessageNOK();
        });
        private readonly Func<WebApiApplicationService.Models.RabbitMQ.MessageModel, MessageModelResponse> _entityDeleteFunc = new Func<Models.RabbitMQ.MessageModel, MessageModelResponse>((x) =>
        {
            return new MessageNOK();
        });
        #endregion
        ...
        internal ILogger<CustomApiControllerBase<T, T2>> Logger
        {
            get
            {
                return _logger;
        ...
        #endregion
        #region Ctor
        public CustomApiControllerBase(ILogger<CustomApiControllerBase<T, T2>> logger, IScopedVulnerablityHandler vulnerablityHandler, IMailHandler mailHandler, IAuthHandler authHandler, IScopedDatabaseHandler databaseHandler, IJsonApiDataHandler jsonApiHandler, ITaskSchedulerBackgroundServiceQueuer queue, IScopedJsonHandler jsonHandler, ICachingHandler cache, IActionDescriptorCollectionProvider actionDescriptorCollectionProvider, IWebHostEnvironment env,IConfiguration configuration, IRabbitMqHandler rabbitMqHandler, IAppconfig appConfig, INodeManagerHandler nodeManagerHandler, IScopedEncryptionHandler scopedEncryptionHandler) :
            base(env, cache, configuration, rabbitMqHandler)
        {
            ...
            _backendModule = (T2)Activator.CreateInstance(typeof(T2), databaseHandler, cache);
            _genericAbstractModelType = (T)Activator.CreateInstance<T>();
        ...
        public CustomApiControllerBase(ILogger<CustomApiControllerBase<T, T2>> logger, IScopedVulnerablityHandler vulnerablityHandler, IMailHandler mailHandler, IAuthHandler authHandler, IScopedDatabaseHandler databaseHandler, IJsonApiDataHandler jsonApiHandler, ITaskSchedulerBackgroundServiceQueuer queue, IScopedJsonHandler jsonHandler, ICachingHandler cache, IWebHostEnvironment env, IConfiguration configuration,IRabbitMqHandler rabbitMqHandler,IAppconfig appConfig, INodeManagerHandler nodeManagerHandler, IScopedEncryptionHandler scopedEncryptionHandler) :
            base(env,cache,configuration, rabbitMqHandler)
        {
            _mailHandler = mailHandler;
            _vulnerablityHandler = vulnerablityHandler;
        ...
        {
            return await this.Get(Guid.Empty.ToString());
        }
        ...
        public virtual async Task<ActionResult<ApiRootNodeModel>> Get(string id, int maxDepth = 1)
        {
            WebApiApplicationService.Models.InternalModels.MethodDescriptor methodInfo = _webHostEnvironment.IsDevelopment() ? new WebApiApplicationService.Models.InternalModels.MethodDescriptor { c = this.GetType().Name, m = MethodBase.GetCurrentMethod().Name } : null;
            ...
            {
                if (queryParamerters.ContainsKey("sort"))
                {
            ...
            AreaAttribute areaAttribute = this.GetArea();
            ...
            if (t.Count != 0)
            {
                ...
                stopwatch.Start();
                apiRootNodeModel = await _jsonApiHandler.CreateApiRootNodeFromModel<T>(areaAttribute.RouteValue, t, maxDepth);
            ...
            if (!String.IsNullOrEmpty(relationname))
            {
                ...
                if (relations != null)
                {
                    ...
                    relations.ForEach(r => {
                        ...
                        {
                            if (abstractModelTmp.DatabaseTable.Replace("_","").ToLower() == relationname.ToLower())
                            {
                ...
                new ApiErrorModel{ Code = ApiErrorModel.ERROR_CODES.HTTP_REQU_RESOURCE_NOT_FOUND, Id = uuid, Detail = "resource not found" }
                    ...
                    ActionResult<ApiRootNodeModel> parent = await this.Get(id);
                    if (parent != null)
                    {
                                    ...
                                    case 200:
                                        if (okObject.Value != null)
                                        {
                                            ...
                                            if (dataModel != null)
                                            {
                                                ...
                                                {
                                                    Related = "/" + this.ControllerName + "/" + id + "/relation/" + relationname + "/" + (relationid == null ?
                                                    "" : relationid),
                                                    Self = "/" + this.ControllerName + "/" + id + "/"
                                                };
        ...
        public virtual async Task<ActionResult<ApiRootNodeModel>> Update(string? id, [FromBody] ApiRootNodeModel body)//model == null wenn formattierung fail, id == 0 wenn int nicht formattiert werden kann wie bsp bei angabe von einem string
        {
            ...
            if (bodyObject != null)
            {
                ...
                ApiRootNodeModel response = await _jsonApiHandler.CreateApiRootNodeFromModel<T>(this.GetArea().RouteKey, itemList);
                ...
                new ApiErrorModel{ Code = ApiErrorModel.ERROR_CODES.HTTP_REQU_BAD, Id = Guid.Empty, Detail = "conflic or badrequest" }
                    ...
                    foreach (ApiDataModel item in bodyValues)
                    {
                        ...
                        {
                            System.Net.HttpStatusCode status = await UpdateAbstractModel(item.Id, item);
                            switch (status)
                            {
                ...
                new ApiErrorModel{ Code = ApiErrorModel.ERROR_CODES.HTTP_REQU_BAD, Id = Guid.Empty, Detail = "not a valid id" }
                    ...
                    foreach (ApiDataModel item in bodyValues)
                    {
                        ...
                        System.Net.HttpStatusCode status = await UpdateAbstractModel(id == null ? item.Id : new Guid(id), item);
                        switch (status)
                        {
                            case System.Net.HttpStatusCode.OK:
        ...
        [NonAction]
        private async Task<System.Net.HttpStatusCode> UpdateAbstractModel(Guid id, ApiDataModel item)
        {
        ...
        public virtual async Task<ActionResult<ApiRootNodeModel>> UpdateRelation(string id, string relationname, [FromBody] ApiRootNodeModel body)
        {
            ...
            if (body != null)
            {
                if (!String.IsNullOrEmpty(relationname))
                {
                    ...
                    var list = SQLDefinitionProperties.BackendTables.Values.ToList();
                                    ...
                                    case 200:
                                        ...
                                        else
                                        {
                                            foreach (ApiDataModel item in bodyValues)
                                            {
                                                ...
                                                {
                                                    var responseSelect = await SqlOp(dataT, item.NetType, SQLDefinitionProperties.SQL_STATEMENT_ART.SELECT);
                                        ...
                                        QueryResponseData<object> currentCollectionFromDatabase = await SqlOp((AbstractModel)relationModelWhereClause, relationModelWhereClause.GetType(), SQLDefinitionProperties.SQL_STATEMENT_ART.SELECT);
                                        ...
                                        switch (method)
                                        {
                                            case "POST":
                                                ...
                                                if (currentCollectionFromDatabase.DataStorage != null)
                                                {
                                                    foreach (object item in currentCollectionFromDatabase.DataStorage)
                                                    {
                                                        ...
                                                        var responseDelete = await SqlOp(model, item.GetType(), SQLDefinitionProperties.SQL_STATEMENT_ART.DELETE);
                                                ...
                                                {
                                                    AbstractModel model = (AbstractModel)item;
                                                    var responseInsert = await SqlOp(model, item.GetType(), SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT);
                                            ...
                                            case "PATCH":
                                                ...
                                                foreach (object item in relationCollectionFromBody)
                                                {
                                                    AbstractModel model = (AbstractModel)item;
                                                    bool canAdd = false;
                                                    var responseSelect = await SqlOp(model, item.GetType(), SQLDefinitionProperties.SQL_STATEMENT_ART.SELECT);
        ...
        public virtual async Task<ActionResult<ApiRootNodeModel>> Create([FromBody] ApiRootNodeModel body)
        {
            WebApiApplicationService.Models.InternalModels.MethodDescriptor methodInfo = _webHostEnvironment.IsDevelopment() ? new WebApiApplicationService.Models.InternalModels.MethodDescriptor { c = this.GetType().Name, m = MethodBase.GetCurrentMethod().Name } : null;
            Logger.TraceHttpTraffic(MethodBase.GetCurrentMethod(), HttpContext, this.ControllerName);
            ApiRootNodeModel bodyObject = body;
            ...
            if (bodyObject != null)
            {
                ...
                ApiRootNodeModel response = await _jsonApiHandler.CreateApiRootNodeFromModel<AbstractModel>(this.GetArea().RouteKey, itemList);
                response.Errors = errors;
                if (response.HasErrors)
                {
                    return JsonApiErrorResult(response, HttpStatusCode.UnprocessableEntity, "an error occurred", "response.HasErrors == true", methodInfo);
                ...
                foreach (ApiDataModel item in bodyValues)
                {
                    ...
                    CreatedAbstractModelResponse<AbstractModel> createdResponse = await CreateAbstractModel(item, bodyObject, transaction);
                    System.Net.HttpStatusCode status = createdResponse.HttpStatusCode;
                ...
                else
                {
                    ...
                    response = await _jsonApiHandler.CreateApiRootNodeFromModel<AbstractModel>(this.GetArea().RouteKey, itemList);
                    ...
                    return Created(this.HttpContext.Request.Path.Value, response);
                }
            }
            return JsonApiErrorResult(new List<ApiErrorModel> {
                new ApiErrorModel{ Code = ApiErrorModel.ERROR_CODES.HTTP_REQU_BAD, Id = Guid.Empty, Detail = "resource not found" }
        ...
        [NonAction]
        private async Task<CreatedAbstractModelResponse<AbstractModel>> CreateAbstractModel(ApiDataModel item, object httpCreatedResponseValue,System.Data.Common.DbTransaction transaction) 
        {
            ...
            if (!existingData.HasData)
            {
                string insertQuery = dataT.GenerateQuery(SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT).ToString();
                ...
                if (data.HasSuccess)
                {
                    ...
                    selectQuery = dataT.GenerateQuery(SQLDefinitionProperties.SQL_STATEMENT_ART.SELECT, dataT, dataT).ToString();
                    ...
                    if (existingData.HasData)
                    {
                        ...
                        responseValue.HttpStatusCode = System.Net.HttpStatusCode.Created;

                    }
                    else
                    {

                        responseValue.HttpStatusCode = System.Net.HttpStatusCode.InternalServerError;
                    }
                }
            }
            else//conflict
            {
                responseValue.HttpStatusCode = System.Net.HttpStatusCode.Conflict;
            }
                                ...
                                if (model.Data != null)
                                {
                                    ...
                                    foreach (ApiDataModel item in targetModels)
                                    {
                                        ...
                                        else
                                        {
                                            ...
                                            errMod.HttpStatus = ((int)System.Net.HttpStatusCode.Conflict).ToString();
                                            response.Errors.Add(errMod);
                                        }
                                    }
        ...
        [NonAction]
        private async Task<QueryResponseData<object>> SqlOp(AbstractModel abstractModelFieldValues,AbstractModel abstractModelWhereClause, Type type, SQLDefinitionProperties.SQL_STATEMENT_ART statementType)
        {

            string query = abstractModelWhereClause.GenerateQuery(statementType, statementType == SQLDefinitionProperties.SQL_STATEMENT_ART.INSERT ? null : abstractModelWhereClause,abstractModelFieldValues).ToString();
