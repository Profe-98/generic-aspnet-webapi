...
namespace WebApiApplicationService.Controllers.APIv1
{
    ...
    public class UserController : CustomApiV1ControllerBase<UserModel, UserModule>
    {
        ...
        public UserController(ILogger<UserController> logger, IScopedVulnerablityHandler vulnerablityHandler, IMailHandler mailHandler, IAuthHandler authHandler, IScopedDatabaseHandler databaseHandler, IActionSelector actionSelector, IJsonApiDataHandler jsonApiHandler, ITaskSchedulerBackgroundServiceQueuer queue, IScopedJsonHandler jsonHandler, ICachingHandler cache, IActionDescriptorCollectionProvider actionDescriptorCollectionProvider, IWebHostEnvironment env,IConfiguration configuration, IScopedEncryptionHandler encryptionHandler,IRabbitMqHandler rabbitMqHandler,IAppconfig appConfig, INodeManagerHandler nodeManagerHandler, IScopedEncryptionHandler scopedEncryptionHandler) :
                    ...
                    switch (okObject.StatusCode)
                    {
                        case 200:
                            ...
                            if(userModel.HasProfilePicture)
                            {
                                ...
                                string userProfilePicPath = Path.Combine(AppPaths.UserProfilePicturesPath, fileName);
        ...
        {
            return await this.Get(Guid.Empty.ToString(),  3);
        }
        ...
        public async Task<ActionResult> RegisterFrontEndUser([FromBody] RegisterDataTransferModel registerModel)
        {
            ...
            else//user existert nicht
            {
                ...
                using (CustomBackendModule<AccountModel> a = new CustomBackendModule<AccountModel>(_databaseHandler, _cacheHandler))
                {
