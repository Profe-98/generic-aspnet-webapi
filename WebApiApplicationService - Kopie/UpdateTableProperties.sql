...
DELIMITER $$
USE `rest_api`$$
DROP TRIGGER IF EXISTS `rest_api`.`site_protect_BEFORE_INSERT` $$
CREATE DEFINER = CURRENT_USER TRIGGER `rest_api`.`site_protect_BEFORE_INSERT` BEFORE INSERT ON `site_protect` FOR EACH ROW
BEGIN
...
DELIMITER $$
USE `rest_api`$$
DROP TRIGGER IF EXISTS `rest_api`.`site_protect_BEFORE_UPDATE` $$
CREATE DEFINER = CURRENT_USER TRIGGER `rest_api`.`site_protect_BEFORE_UPDATE` BEFORE UPDATE ON `site_protect` FOR EACH ROW
BEGIN
